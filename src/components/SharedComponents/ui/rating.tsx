...
import { cloneElement, useCallback, useId, useMemo, useState } from 'react'
...
function RatingItem({
  ...
  const icons = useMemo(() => {
    ...
    const fullIcon = cloneElement(Icon, {
      size,
  ...
  const getRatingPoint = useCallback(
...
function Rating({
  ...
  const handleValueHover = useCallback(
    (point: number) => {
